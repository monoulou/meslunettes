...
class ConnectController extends Controller
{
    ...
    public function connectAction(Request $request)
    {
        ...
        if ($request->attributes->has($authErrorKey)) {
            $fosError = $request->attributes->get($authErrorKey);
        } elseif (null !== $session && $session->has($authErrorKey)) {
            $fosError = $session->get($authErrorKey);
            $session->remove($authErrorKey);
        } else {
            $fosError = null;
        }

        if (!$fosError instanceof AuthenticationException) {
            $fosError = null; // The value does not come from the security component.
        }
